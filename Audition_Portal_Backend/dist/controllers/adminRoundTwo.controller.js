...
exports.getRoundTwoCandidatesByPanel = exports.forwardRoundTwoCandidate = exports.evaluateRoundTwoCandidate = exports.getRoundTwoStatistics = exports.updateRoundTwoTask = exports.submitRoundTwoReview = exports.getRoundTwoCandidateDetails = exports.fetchAllRoundTwoCandidates = void 0;
...
exports.fetchAllRoundTwoCandidates = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, candidates, "Round 2 candidates fetched successfully"));
});
// Get candidate personal details for Round 2
exports.getRoundTwoCandidateDetails = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!candidate.round || candidate.round < 2) {
        throw new ApiError_1.ApiError(400, "Candidate is not in Round 2");
    }
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, candidate, "Round 2 candidate details fetched successfully"));
});
// Submit or update Round 2 review
exports.submitRoundTwoReview = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    const { userId, roundTwoId, attendance, taskgiven, clubPrefer, subDomain, hs_place, reviews, remarks, rating, gd, general, forwarded, // optional: accept forwarded flag from client but do NOT treat it as final decision
    colour, } = req.body;
    // Validate required fields
    if (!userId || !roundTwoId) {
        throw new ApiError_1.ApiError(400, "userId and roundTwoId are required");
    }
    if (typeof rating !== "number" || rating < 0 || rating > 10) {
        throw new ApiError_1.ApiError(400, "Rating must be a number between 0 and 10");
    }
    ...
    if (!user.round || user.round < 2) {
        throw new ApiError_1.ApiError(400, "User is not in Round 2");
    }
    if (!user.roundTwo) {
        throw new ApiError_1.ApiError(400, "User does not have Round 2 data");
    }
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, { ...result, forwarded: forwardedFlag, colour: colour || undefined }, "Round 2 review submitted successfully"));
});
// Update Round 2 task assignment
exports.updateRoundTwoTask = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    const { userId } = req.params;
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, updatedRoundTwo, "Round 2 task updated successfully"));
});
// Get Round 2 statistics
exports.getRoundTwoStatistics = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, statistics, "Round 2 statistics fetched successfully"));
});
// Accept or Reject a Round 2 candidate
exports.evaluateRoundTwoCandidate = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    const { userId, finalSelection, remarks, } = req.body;
    // Validate required fields
    if (!userId || typeof finalSelection !== "boolean") {
        throw new ApiError_1.ApiError(400, "userId and finalSelection (boolean) are required");
    }
    ...
    if (round2AuditionRound) {
        throw new ApiError_1.ApiError(400, "User has already been evaluated for Round 2");
    }
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, result, message));
});
// Forward or Not Forward a Round 2 candidate (after review)
exports.forwardRoundTwoCandidate = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    const { userId, forwarded, remarks, } = req.body;
    // Validate required fields
    if (!userId || typeof forwarded !== "boolean") {
        throw new ApiError_1.ApiError(400, "userId and forwarded (boolean) are required");
    }
    // Get evaluator's email from authenticated user
    const adminUser = req.user;
    const reviewedBy = adminUser?.email || "admin@domain.com";
    ...
    if (!user.roundTwo.review) {
        throw new ApiError_1.ApiError(400, "User has not been reviewed yet. Please submit a review first.");
    }
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, review, message));
});
// Get candidates by panel for Round 2
exports.getRoundTwoCandidatesByPanel = (0, AsyncHandler_1.asyncHandler)(async (req, res) => {
    const { panel } = req.params;
    if (!panel || typeof panel !== "string") {
        throw new ApiError_1.ApiError(400, "Invalid or missing panel number");
    }
    const panelNumber = Number(panel);
    if (isNaN(panelNumber) || panelNumber < 1 || panelNumber > 6) {
        throw new ApiError_1.ApiError(400, "Panel number must be between 1 and 6");
    }
    const candidates = await prisma.user.findMany({
        where: {
            role: client_1.Role.USER,
    ...
    return res
        .status(200)
        .json(new ApiResponse_1.ApiResponse(200, candidates, `Panel ${panelNumber} candidates fetched successfully`));
});
